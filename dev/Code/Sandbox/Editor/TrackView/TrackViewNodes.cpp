...
void CTrackViewNodesCtrl::AddGroupNodeAddItems(SContextMenu& contextMenu, CTrackViewAnimNode* animNode)
{
    ...
    {
        contextMenu.main.addAction(tr("Add Current Layer"))->setData(eMI_AddCurrentLayer);
    }
    ...
    {
        contextMenu.main.addAction(tr("Add Environment Node"))->setData(eMI_AddEnvironment);
    }
    ...
    contextMenu.main.addAction(tr("Add Material Node"))->setData(eMI_AddMaterial);
...
int CTrackViewNodesCtrl::ShowPopupMenuSingleSelection(SContextMenu& contextMenu, CTrackViewSequence* sequence, CTrackViewNode* pNode)
{
    ...
    if (bOnNode && !bIsLightAnimationSet && (animNode->GetEntity() != nullptr || animNode->IsBoundToAzEntity()))
    {
        ...
        {
            contextMenu.main.addAction(tr("Set As View Camera"))->setData(eMI_SetAsViewCamera);
        }
    ...
    if (bOnSequence || pNode->IsGroupNode())
    {
        ...
        if (!isOnAzEntity)
        {
            ...
            contextMenu.expandSub.addAction(tr("Expand Cameras"))->setData(eMI_ExpandCameras);
            contextMenu.collapseSub.addAction(tr("Collapse Cameras"))->setData(eMI_CollapseCameras);
            contextMenu.expandSub.addAction(tr("Expand Materials"))->setData(eMI_ExpandMaterials);
            contextMenu.collapseSub.addAction(tr("Collapse Materials"))->setData(eMI_CollapseMaterials);
        ...
        if (bOnNode)
        {
            ...
            if (!isOnAzEntity)
            {
                ...
                if (FillAddTrackMenu(contextMenu.addTrackSub, animNode))
                {
                    ...
                    CreateAddTrackMenuRec(contextMenu.main, tr("Add Track"), animNode, contextMenu.addTrackSub, currentId);
                }
            }
    ...
    {
        AddMenuSeperatorConditional(contextMenu.main, bAppended);
        QString string = QString(tr("%1 Tracks")).arg(animNode->GetName());
...
void CTrackViewNodesCtrl::CreateSetAnimationLayerPopupMenu(QMenu& menuSetLayer, CTrackViewTrack* pTrack) const
{
    ...
    for (int i = 0; i < 16; ++i)
    {
        QString layerText = QString(tr("Layer #%1")).arg(i);
