...
bool EDITOR_COMMON_API ShowBatchFileDialog(Serialization::StringList* result, const SBatchFileSettings& settings, QWidget* parent)
{
    ...
    if (settings.scanExtension[0] != '\0')
    {
        if (settings.useCryPak)
        {
            ...
            helper.ScanDirectoryRecursive(Path::GetEditingGameDataFolder().c_str(), "", mask.c_str(), files);
        ...
        else
        {
            ...
            do
            {
                ...
                QDirIterator dirIterator(QString(modPrefix), QStringList() << mask, QDir::Files, QDirIterator::Subdirectories);
