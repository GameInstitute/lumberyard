...
bool QPropertyTree::onRowLMBDown(PropertyRow* row, const QRect& rowRect, QPoint point, bool controlPressed, bool shiftPressed)
{
	pressPoint_ = point;
...
void QPropertyTree::onModelPushRedo(PropertyTreeOperator* op, bool* handled)
{
	signalPushRedo();
}
...
bool QPropertyTree::RowFilter::match(const char* textOriginal, Type type, size_t* matchStart, size_t* matchEnd) const
{
	...
	const string &start = this->start[type];
	if (tillEnd[type]){
		if (start == text) {
			if (matchStart)
				*matchStart = 0;
	...
	const vector<string> &substrings = this->substrings[type];
...
#include <QPropertyTree/QPropertyTree.moc>
#include <QPropertyTree/PropertyTreeMenuHandler.moc>
