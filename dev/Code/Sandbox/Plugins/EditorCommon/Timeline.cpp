...
namespace
{
    ...
    void DrawTracks(QPainter& painter, const uint startPass, const uint endPass, const STimelineLayout& layout, const STimelineViewState& viewState,
        const QPalette& palette, const QPoint& mousePos, bool hasFocus, int width, float keyRadius, float timeUnitScale, bool drawMarkers)
    {
            ...
            for (size_t j = 0; j < tracks.size(); ++j)
            {
                ...
                if (pass == PASS_BACKGROUND)
                {
                    ...
                    if (bIsToggleTrack)
                    {
                        ...
                        for (uint i = drawStart; i <= numElements; i += 2)
                        {
                            const int left = (i == 0) ? (-viewState.scrollPixels.x()) : sortedElements[i - 1].rect.right();
                            const int right = (i == numElements) ? (-viewState.scrollPixels.x() + width) : sortedElements[i].rect.left();
    ...
    void DrawTree(QPainter& painter, const QRect& treeRect, const QPalette& palette, QWidget* timeline, const STimelineContent& content, const STrackLayouts& tracks, const STimelineViewState& viewState, int scroll)
    {
        painter.save();
...
void CTimeline::mousePressEvent(QMouseEvent* ev)
{
    ...
    else if (ev->button() == Qt::RightButton)
    {
        ...
        else
        {
            ...
            ForEachElement(m_pContent->track, [&](STimelineTrack& t, STimelineElement& e)
                {
                    if (e.selected)
                    {
                        hasSelection = true;
