    ...
    , m_canvasScaleLabel(new QLabel(parent))
{
    ...
    addSeparator();
    m_viewportSizeLabel->setToolTip(QString(tr("The current size of the viewport")));
    ...
    addSeparator();
    m_canvasScaleLabel->setToolTip(QString(tr("The scale used to fit the canvas in the viewport")));
    ...
    m_editButton->setToolTip(QString(tr("Switch back to Edit mode")));
...
{
    m_viewportSizeLabel->setText(QString(tr("Viewport size: %1 x %2")).arg(QString::number(ev->size().width()),
            QString::number(ev->size().height())));
}

void PreviewToolbar::UpdatePreviewCanvasScale(float scale)
{
    m_canvasScaleLabel->setText(QString(tr("Canvas scale: %1%")).arg(QString::number(scale * 100.0f)));
}
