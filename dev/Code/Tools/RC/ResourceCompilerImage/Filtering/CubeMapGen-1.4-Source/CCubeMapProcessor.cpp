    ...
    int32 a_SampleCountGGX)
{
   ...
   else //otherwise, split work between callers thread, and newly created thread
   {
      ...
      //start thread 1
       m_ThreadHandle[1] = AZStd::thread(AZStd::bind(ThreadProcCPU1Filter, (LPVOID)NULL));
      ...
      //spin and sleep until thread1 is completed and we haven t received any signal to shutdown before continuing
      while( IsFilterThreadActive(1) && !m_shutdownWorkerThreadSignal)
      {         
...
{   
   ...
   if(m_NumFilterThreads > 0)
   {
      ...
      sg_FilterOptionsCPU0.m_bUseSolidAngle = a_bUseSolidAngle;
       ...
       m_ThreadHandle[0] = AZStd::thread(AZStd::bind(ThreadProcCPU0Filter, (LPVOID)NULL));
