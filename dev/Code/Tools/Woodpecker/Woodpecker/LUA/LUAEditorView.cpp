...
#include <Woodpecker/LUA/LUAEditorView.moc>
...
namespace LUAEditor
{
    ...
    void LUAViewWidget::FindMatchingBrace(Callable callable)
    {
        ...
        detect();
        if (braceChar == '\0' && bracePos > 0) //try previous char too so we can detect on either side of brace
        {
            --bracePos;
            detect();
        }
        if (braceChar != '\0') //found one
        {
