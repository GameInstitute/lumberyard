...
namespace EMStudio
{
    ...
    bool MotionSetWindow::FillRow(EMotionFX::MotionSet* motionSet, EMotionFX::MotionSet::MotionEntry* motionEntry, uint32 rowIndex, QTableWidget* tableWidget, bool readOnly)
    {
        ...
        if (entryMotion)
        {
            tempString = AZStd::string::format("%.2f sec", entryMotion->GetMaxTime());
        }
        else
        {
            tempString = "";
        ...
        if (entryMotion)
        {
            if ((entryMotion->GetType() == EMotionFX::SkeletalMotion::TYPE_ID) || (entryMotion->GetType() == EMotionFX::WaveletSkeletalMotion::TYPE_ID))
            {
                EMotionFX::SkeletalMotion* skeletalMotion = static_cast<EMotionFX::SkeletalMotion*>(entryMotion);

                // create the sub entry
                AZStd::to_string(tempString, skeletalMotion->GetNumSubMotions());
                ...
                AZStd::to_string(tempString, skeletalMotion->GetNumMorphSubMotions());
        ...
        if (entryMotion)
        {
            QTableWidgetItem* typeTableItem = new QTableWidgetItem(entryMotion->GetTypeString());
    ...
    bool MotionSetWindow::RemoveRow(EMotionFX::MotionSet* motionSet, EMotionFX::MotionSet::MotionEntry* motionEntry, QTableWidget* tableWidget)
    {
        if (!motionEntry)
        {
            return false;
    ...
    void MotionSetWindow::UpdateMotionSetTable(QTableWidget* tableWidget, EMotionFX::MotionSet* motionSet, bool readOnly)
    {
        ...
        for (const auto& item : motionEntries)
        {
            ...
            if (motion)
            {
                tempString = AZStd::string::format("%.2f sec", motion->GetMaxTime());
            }
            else
            {
                tempString = "";
...
#include <EMotionFX/Tools/EMotionStudio/Plugins/StandardPlugins/Source/MotionSetsWindow/MotionSetWindow.moc>